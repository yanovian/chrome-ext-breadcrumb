.PHONY: help install prepare assets dev dev-firefox build build-firefox \
	...
	website-install website-prepare website-dev website-build website-preview website-clean \
	website-lint-i18n website-lint-i18n-fix website-og-images

PNPM ?= pnpm
WEBSITE ?= website
# Shortest GitHub Pages path for yanovian/chrome-ext-breadcrumb (repo name, no extra folder).
GITHUB_PAGES_BASE ?= /chrome-ext-breadcrumb/
...
check: typecheck test build website-build ## CI-style check: typecheck, test, build, website
...
website-prepare: ## Generate .wxt/tsconfig.json only, root tsconfig.json extends it, skips the extension's model/wasm postinstall assets
